...
const WatchlistSidebar = ({ onClose, isMobile = false }) => {
  ...
  const refreshWatchlists = useCallback(async (preferredName) => {
    try {
      ...
      console.error("Unable to refresh watchlists", error);
    } finally {
  ...
  useEffect(() => {
    ...
    const seedPrices = async () => {
      try {
        ...
        console.error("Failed to seed watchlist prices", error);
      }
    };
    seedPrices();
    return () => {
      ...
      <div className="space-y-3">
        <div className="relative">
          ...
          <Input
            ...
            className="rounded-2xl border-slate-200 bg-slate-50 pl-10 text-sm"
          />
        ...
        {isLoadingWatchlists ? (
          ...
          displayedStocks.map((stock) => {
            ...
            return (
              <div
                key={stock.symbol}
                className="cursor-pointer rounded-3xl border border-slate-100 bg-white p-4 shadow-sm transition hover:shadow-md"
                onClick={() => handleRowClick(stock.symbol)}
              >
                <div className="flex min-w-0 flex-wrap items-start gap-3">
                  <div className="min-w-0 flex-1">
                    <p className="text-base font-semibold text-slate-900">{stock.symbol?.split(".")[0]}</p>
                    <p className="text-xs text-slate-500 text-balance leading-snug md:line-clamp-2">{stock.name || "--"}</p>
                  </div>
                  <div className="text-right whitespace-nowrap sm:min-w-[120px]">
                ...
                <div className="mt-4 flex flex-wrap items-center gap-2 sm:justify-end">
                  ...
                  <Button
                    ...
                    size="sm"
                    variant="ghost"
                    className="rounded-full border border-slate-200 bg-white text-slate-600 hover:text-slate-900"
                    onClick={(event) => {
                      event.stopPropagation();
...
const SidebarRowSkeleton = () => (
  <div className="animate-pulse rounded-3xl border border-slate-100 bg-white p-4">
