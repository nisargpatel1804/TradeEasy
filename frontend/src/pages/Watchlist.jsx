...
const Watchlist = () => {
    ...
    // --- Sparkline Fetch Effect ---
    useEffect(() => {
        const symbolsToFetch = stocksWithLiveData
            .map(s => s.symbol)
            .filter(sym => sparklines[sym] === undefined); // CRITICAL: Check strictly for undefined
        ...
        const fetchSparklines = async () => {
            try {
                ...
                if (res?.success && res?.sparklines) {
                    ...
                    // CRITICAL: On failure, set empty arrays so it stops retrying
                    const fallbacks = {};
                ...
                // CRITICAL: On 404/Error, set empty arrays to prevent DDOSing the server
                const fallbacks = {};
                symbolsToFetch.forEach(sym => { fallbacks[sym] = []; });
                setSparklines(prev => ({ ...prev, ...fallbacks }));
            }
...
const StockRow = ({ stock, sparklineData, onTrade, onRemove }) => {
    const navigate = useNavigate();
...
const CompactStockCard = ({ stock, sparklineData, onTrade, onRemove }) => {
    const navigate = useNavigate();
    const toNumber = (value, fallback = 0) => {
        if (typeof value === 'number' && Number.isFinite(value)) {
            return value;
