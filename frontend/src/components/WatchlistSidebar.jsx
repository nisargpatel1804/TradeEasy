...
const WatchlistSidebar = ({ onClose, isMobile = false }) => {
  ...
  // Fetch sparklines for displayed stocks with a debounce
  useEffect(() => {
    const symbolsToFetch = displayedStocks
      .map(s => s.symbol)
      .filter(sym => sparklines[sym] === undefined); // CRITICAL: Check strictly for undefined to prevent loop on failure

    if (symbolsToFetch.length === 0) return;

    const fetchSparklines = async () => {
      try {
        const res = await api.getBatchSparklines(symbolsToFetch);
        if (res?.success && res?.sparklines) {
          setSparklines(prev => ({ ...prev, ...res.sparklines }));
        } else {
          // Prevent infinite loops on structured failure
          const fallbacks = {};
          symbolsToFetch.forEach(sym => { fallbacks[sym] = []; });
          setSparklines(prev => ({ ...prev, ...fallbacks }));
        }
      } catch (e) {
        console.error("Failed to load sparklines", e);
        // CRITICAL: On 404/Error, mark as empty to prevent infinite loops & DDOSing the server
        const fallbacks = {};
        symbolsToFetch.forEach(sym => { fallbacks[sym] = []; });
        setSparklines(prev => ({ ...prev, ...fallbacks }));
      }
    ...
    // 500ms debounce prevents API spam while sorting/searching
    const timer = setTimeout(fetchSparklines, 500);
    return () => clearTimeout(timer);
  }, [displayedStocks, sparklines]);
            ...
            return (
              ...
              >
                <div className="flex min-w-0 items-center gap-3">
                  ...
                  <div className="hidden sm:block flex-shrink-0 opacity-70 group-hover:opacity-100 transition-opacity pr-2">
                    <Sparkline data={sparklines[stock.symbol]} width={45} height={20} />
                  </div>
